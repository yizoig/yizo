...
let OrderOfferHelpModel = require("../model/orderOfferHelp")
let OrderController = require("./order");
module.exports = class OrderOfferHelpController extends Controller {

    /**
     * 添加多用户跑单
     */
    async add({title, content, gender_constraint, college, maxNum }) {

        let model = new OrderOfferHelpModel();
        let order_id = OrderController.makeId("M");


        console.log(order_id)
        let result = await model.add({ title,order_id, content, gender_constraint, college, max_number: maxNum }, this.reqUser);
        return this.json({
            data: result
        })
    }
