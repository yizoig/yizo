...
P('home', {
    ...
    orderTask: function (e) {
        ...
        request('grabFindOrder', {id: detail['order_id']}).then(({data}) => {
            if(!data){
                this.$showToast({
                    title:"抢单失败",
                    icon:'error'
            ...
            list[currentTab]['items'][index]['runner'] = userInfo['id'];
            list[currentTab]['items'][index]['runner_name'] = userInfo['nickname'];
            list[currentTab]['items'][index]['runner_gender'] = userInfo['gender'];
