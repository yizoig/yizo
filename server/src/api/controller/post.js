...
module.exports = class Post extends JikeJs.Controller {
    ...
    async commentList({ search, creater, id: postId, page, pageSize, _d }) {
    ...
    async commentAdd({ id, content }) {
        ...
        let model = new CommentModel();
        return await model.add(id, { content }, this.user.id);
    }
    async commentUpdate({ content, id, cid }) {

        let model = new CommentModel();
        return await model.updateInfo({ id, cid, content });
    }
    async commentDel({ id, cids }) {
